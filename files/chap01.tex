...
\section{인용해 보기}
제 \ref{ss:normal}절의 식 (\ref{eq:eq1})\은 정규분포일 때의 확률밀도함수이며 그림 \ref{fig:play}\와는 아무 관계가 없다. \pageref{eq:eq1}면의 식 (\ref{eq:eq1})의 특수한 경우로 $\mu=0$, $\sigma=1$이면 식 (\ref{eq:eq2})가 된다. 제 \ref{ss:add}절은 \pageref{ss:add}면에 나온다.
